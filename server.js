...
io.on('connection', (socket) => {
  ...
  socket.on('join', (data) => {
    ...
    // Eyni uid yeni socket ilə yenidən qoşulursa — köhnə auto_spin timer-ını sıfırla
    // Yoxsa timer bitib sırası olmayan kişiyə auto_spin gedər
    ...
    if (existingPlayer && existingPlayer.socketId !== socket.id) {
      ...
      if (ts.current === uid && ts.timer) {
        ...
        // Qısa gecikmə ilə yenidən your_turn göndər — client yeni socket ilə hazır olsun
        setTimeout(() => {
          const ts2 = getTurn(mid);
          if (ts2.current === uid) {
  ...
  socket.on('spin', (data) => {
    ...
    const mid = String(data.masaId);
    const ts  = getTurn(mid);
    // Sırası olmayan biri spin göndərərsə yoksay — həm güvenlik həm turn bütünlüyü
    const entry = socketRoom[socket.id];
    if (entry && ts.current !== null && ts.current !== String(entry.uid)) return;
    if (ts.timer) { clearTimeout(ts.timer); ts.timer = null; }
    io.to(mid).emit('spin', data);
  });
  ...
  socket.on('next_turn_ready', (data) => {
    ...
    // NOT: if(ts.pending) return yerine clearTimeout+yenile — yoksa p1 drop'da oyun takılır
  ...
  socket.on('auto_spin',      ()  => {}); // server idarə edir — client relay edilmir (güvenlik)
